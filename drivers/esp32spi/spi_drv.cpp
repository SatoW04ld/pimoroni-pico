...
namespace pimoroni {
  ...
  int SpiDrv::send_command(uint8_t command, const SpiDrv::inParam *params_in, uint8_t num_in, uint8_t *data, uint16_t *data_len, cmd_response_type response_type) {
    wait_for_esp_select();

    WARN("\n%s %d\n", commands[command], num_in);

    // Send Command
    send_cmd(command, num_in);

    // Send params
    for(uint8_t i = 0; i < num_in; i++) {
      SpiDrv::inParam param = params_in[i];
      SpiDrv::lastParam last = (i == num_in - 1) ? SpiDrv::LAST_PARAM : SpiDrv::NO_LAST_PARAM;
      switch(param.type) {
        case PARAM_NORMAL:
          WARN("param %d\n", param.len);
          send_param(param.addr, param.len, last);
          break;
        case PARAM_BUFFER:
          WARN("buffer %d\n", param.len);
          send_buffer(param.addr, param.len, last);
          break;
        case PARAM_DUMMY:
          WARN("dummy\n");
          uint8_t dummy = DUMMY_DATA;
          send_param(&dummy, 1, last);
          break;
      }
    }
    esp_deselect();
  ...
  int SpiDrv::send_command(uint8_t command, const SpiDrv::inParam *params_in, uint8_t num_in) {
    uint8_t data;
    uint16_t data_len = 0;
    send_command(command, params_in, num_in, &data, &data_len);
    return data;
  }

  int SpiDrv::send_command(uint8_t command, SpiDrv::tParam *params_out, SpiDrv::numParams num_out) {
    wait_for_esp_select();

    // Send Command
    send_cmd(command, SpiDrv::PARAM_NUMS_1);
    uint8_t dummy = DUMMY_DATA;
    send_param(&dummy, 1, SpiDrv::LAST_PARAM);
    ...
    // Wait for reply
    wait_for_esp_select();
    wait_response_params(command, num_out, params_out);
    esp_deselect();

    return num_out;
  }

  int SpiDrv::send_command(uint8_t command, uint8_t *data, uint16_t *data_len, bool dummy) {
    SpiDrv::inParam params[] = {
      SpiDrv::build_param_dummy()
    };
    return send_command(command, params, dummy ? 1 : 0, data, data_len);
  }

  int SpiDrv::send_command(uint8_t command, bool dummy) {
    uint8_t data;
    uint16_t data_len;
    send_command(command, &data, &data_len, dummy);
    return data;
  }
}